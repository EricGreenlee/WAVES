...
void setup()
{
  ...
  SD.remove("/swarmPP.txt");
  SD.remove("/swarmTLE.txt");
  SD.remove("/mySwarm.txt");
  if (!SD.begin(sd_cs)) {
  ...
  if (!ppFile)
  {
    Serial.println(F("Could not open swarmPP.txt for writing! trying again..."));
    while (!ppFile){
      
      ppFile = SD.open("/swarmP.txt", FILE_WRITE);
      delay(500);
      Serial.println(F("Could not open swarmPP.txt for writing! trying again..."));
    }
    Serial.println(F("Opened swarmPP.txt!!!"));
  }  
  //ppFile.print("This is swarmPP.txt\n");
  

  
  File tleFile = SD.open("/swarmTLE.txt", FILE_WRITE);//SD.open("/swarmTLE.txt", FILE_WRITE);
  if (!tleFile)
  {
   ...
   //tleFile.print("Hello this is swarmTLE.txt\n");
